...
# ── Plotly export (PNG con fallback, y HTML interactivo) ──────────────────────
def kaleido_disponible() -> bool:
    """True si se puede exportar imagen estática (kaleido instalado y operativo)."""
    try:
...
def export_gantt_png(fig):
    """Devuelve (bytes, ext). Intenta PNG; si kaleido falla, cae a HTML interactivo.
    Revisa la extensión devuelta para nombrar el archivo de descarga."""
    h = int(fig.layout.height or 800)
    try:
        return fig.to_image(format="png", width=1800, height=max(600, h), scale=2), "png"
    except Exception:
        return export_gantt_html(fig), "html"


def export_gantt_html(fig) -> bytes:
    """Gantt como HTML interactivo (hover + zoom). No requiere kaleido ni Chrome."""
