...
from modules.exportar import export_checklist_word, export_gantt_excel
...
# ════════════════════════════════════════════════════════════════════════════════
data  = data_loader.load(st.session_state.tech_id)
...
if vista == "📅 Línea de Tiempo":
    ...
    with col_b:
        try:
            png = fig.to_image(format='png', width=1800, height=max(600, fig.layout.height or 800), scale=2)
            st.download_button("🖼️ Exportar PNG", data=png,
                file_name=f"LUXEM_Gantt_{st.session_state.tech_id}.png", mime="image/png")
        except Exception:
            st.caption("PNG: requiere `pip install kaleido`")
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    da = data_loader.load(ids[idx_a])
    db = data_loader.load(ids[idx_b])
